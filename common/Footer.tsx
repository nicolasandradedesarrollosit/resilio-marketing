'use client';

import Image from 'next/image';
import { Link } from '@heroui/react';

export default function Footer() {
  const anioActual = new Date().getFullYear();

  return (
    <footer className="bg-black text-white w-full border-t border-gray-900">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 md:px-8 lg:px-16 py-12 sm:py-16 md:py-20">
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-8 sm:gap-12 md:gap-16 mb-12 sm:mb-16">
          <div className="flex flex-col gap-4 sm:gap-6">
            <div className="flex items-center gap-3">
              <Image
                alt="Logo de Resilio"
                className="h-10 w-10 sm:h-12 sm:w-12"
                height={48}
                src="/logo-icon.png"
                width={48}
              />
              <span className="text-xl sm:text-2xl font-light tracking-tight">
                Resilio
              </span>
            </div>
            <p className="text-sm sm:text-base text-gray-400 font-light leading-relaxed max-w-xs">
              Una comunidad de jóvenes amantes de la creatividad y las buenas
              experiencias.
            </p>
          </div>

          <div className="flex flex-col gap-4 sm:gap-5">
            <h3 className="text-sm sm:text-base font-medium text-white/90 mb-2">
              Seguinos
            </h3>
            <div className="flex gap-4">
              <Link
                aria-label="Seguinos en Instagram (se abre en nueva pestaña)"
                className="w-10 h-10 rounded-full bg-gray-900 flex items-center justify-center hover:bg-dull-lavender-600 transition-all duration-300 group"
                href="https://www.instagram.com/resilio.life/"
                rel="noopener noreferrer"
                target="_blank"
              >
                <svg
                  aria-hidden="true"
                  className="w-5 h-5 text-gray-400 group-hover:text-white transition-colors"
                  fill="currentColor"
                  viewBox="0 0 24 24"
                >
                  <path d="M12 2.163c3.204 0 3.584.012 4.85.07 3.252.148 4.771 1.691 4.919 4.919.058 1.265.069 1.645.069 4.849 0 3.205-.012 3.584-.069 4.849-.149 3.225-1.664 4.771-4.919 4.919-1.266.058-1.644.07-4.85.07-3.204 0-3.584-.012-4.849-.07-3.26-.149-4.771-1.699-4.919-4.92-.058-1.265-.07-1.644-.07-4.849 0-3.204.013-3.583.07-4.849.149-3.227 1.664-4.771 4.919-4.919 1.266-.057 1.645-.069 4.849-.069zm0-2.163c-3.259 0-3.667.014-4.947.072-4.358.2-6.78 2.618-6.98 6.98-.059 1.281-.073 1.689-.073 4.948 0 3.259.014 3.668.072 4.948.2 4.358 2.618 6.78 6.98 6.98 1.281.058 1.689.072 4.948.072 3.259 0 3.668-.014 4.948-.072 4.354-.2 6.782-2.618 6.979-6.98.059-1.28.073-1.689.073-4.948 0-3.259-.014-3.667-.072-4.947-.196-4.354-2.617-6.78-6.979-6.98-1.281-.059-1.69-.073-4.949-.073zm0 5.838c-3.403 0-6.162 2.759-6.162 6.162s2.759 6.163 6.162 6.163 6.162-2.759 6.162-6.163c0-3.403-2.759-6.162-6.162-6.162zm0 10.162c-2.209 0-4-1.79-4-4 0-2.209 1.791-4 4-4s4 1.791 4 4c0 2.21-1.791 4-4 4zm6.406-11.845c-.796 0-1.441.645-1.441 1.44s.645 1.44 1.441 1.44c.795 0 1.439-.645 1.439-1.44s-.644-1.44-1.439-1.44z" />
                </svg>
              </Link>
            </div>
          </div>
        </div>

        <div
          aria-hidden="true"
          className="w-full h-px bg-gradient-to-r from-transparent via-gray-800 to-transparent mb-8 sm:mb-10"
        />

        <div className="flex flex-col sm:flex-row justify-between items-center gap-4 sm:gap-6">
          <p className="text-xs sm:text-sm text-white font-light text-center sm:text-left">
            &copy; {anioActual} Resilio. Todos los derechos reservados.
          </p>
          <p className="text-xs sm:text-sm text-white font-light text-center sm:text-right">
            Desarrollado por{' '}
            <Link
              className="text-gray-400 hover:text-dull-lavender-400 transition-colors duration-300 text-xs sm:text-sm"
              href="https://andart.company"
              rel="noopener noreferrer"
              target="_blank"
            >
              Andart
              <span className="sr-only"> (se abre en nueva pestaña)</span>
            </Link>
          </p>
        </div>
      </div>
    </footer>
  );
}
